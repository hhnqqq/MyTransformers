...
class LongRopeDataset(Dataset):
    ...
    def preprocess_sample(self, sample):
        ...
        if self.mode == 'sft':
            ...
            input_text = self.meta_prompt + self.prefix + sample["input"] + '\n'
            output_text = self.postfix + sample["output"]
            ...
            if isinstance(sample, dict):
                ...
                input = self.meta_prompt + self.prefix + sample["input"] + self.postfix
                input += sample["output"] if "output" in sample.keys() else ''
            elif isinstance(sample, str):
                input = self.meta_prompt + self.prefix + sample + self.postfix
            else:
