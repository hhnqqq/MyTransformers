...
# name=train_clip_vera_1gpu
name=vera_share_rank8
...
do
  ...
  do
    lora_options="--use-lora \
      ...
      --weight-b-init-method kaiming \
      --lambda-b-init-method zero \
      --lambda-d-init-method small_constant \
      --use-lora-plus \
      --lora-plus-scaler 1 \
      "
    ...
    options="--experiment-name ${name}_${dataset}_seed${seed} \
      ...
      --output-path /fs-computility/mabasic/yepeng/liminglei/MyTransformers/checkpoint/clip_vera_finetuned \
      --tensorboard \
